...
.DEFAULT_GOAL := help
.PHONY = fmt vet all pack clean deps update init vendor mod-init mod-update mod-download mod-vendor help

################################################################################

all: ## Build binary
	@echo "[36;1mBuilding function…[0m"
	@go build ycfunc.go

pack: ## Pack function into zip
	@echo "[36;1mPacking YC function to zip…[0m"
	@go build ycfunc.go && rm -f ycfunc
	@zip funky -r "ycfunc.go" go.*
